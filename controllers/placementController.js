...
export const addPlacementData = async (req, res) => {
  try {
    const { collegeId, salaryRange, studentsPlaced, companiesVisited } = req.body;

    if (!collegeId || !salaryRange || !studentsPlaced || !companiesVisited) {
      return res.status(400).json({ message: "All fields (collegeId, salaryRange, studentsPlaced, companiesVisited) are required" });
    }
    ...
    res.status(201).json({ message: "Placement data added successfully", placement });
  } catch (error) {
...
export const getPlacementByCollege = async (req, res) => {
  try {
    ...
    if (!placements.length) {
      return res.status(404).json({ message: "No placement data found for this college" });
    }
...
export const updatePlacement = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Placement data updated", placement: updatedPlacement });
  } catch (error) {
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};
...
export default { addPlacementData, getPlacementByCollege, updatePlacement, deletePlacement };
