...
export const addAdmissionProcess = async (req, res) => {
  try {
    const { collegeId, admissionCriteria, applicationProcess, importantDates, documentsRequired, selectionProcess } = req.body;

    if (!collegeId || !admissionCriteria || !applicationProcess || !importantDates || !documentsRequired || !selectionProcess) {
...
export const updateAdmissionProcess = async (req, res) => {
  try {
    ...
    const { admissionCriteria, applicationProcess, importantDates, documentsRequired, selectionProcess } = req.body;

    const updatedAdmissionProcess = await AdmissionProcess.findByIdAndUpdate(
      admissionProcessId,
      { admissionCriteria, applicationProcess, importantDates, documentsRequired, selectionProcess },
